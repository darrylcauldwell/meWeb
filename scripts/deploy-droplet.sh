...
# ---------------------------------------------------------------------------
# 6. Write .env (only on first run - preserves existing secrets)
# ---------------------------------------------------------------------------
ENV_FILE="$DEPLOY_DIR/site/.env"
if [[ -f "$ENV_FILE" ]]; then
  log "$ENV_FILE already exists, skipping (delete it and re-run to regenerate)."
else
...
SUPPORT_ADDRESS=support.dreamfold.dev
...
# ---------------------------------------------------------------------------
log "Pulling images and starting production stack..."
cd "$DEPLOY_DIR/site"
docker compose -f docker-compose.production.yml pull
docker compose -f docker-compose.production.yml up -d

# ---------------------------------------------------------------------------
# 8. Seed EVM with demo data (first run only - idempotent)
# ---------------------------------------------------------------------------
...
if curl -sf --max-time 10 "http://localhost/healthz" > /dev/null 2>&1; then
  ...
  log "Health endpoint not yet reachable (normal if DNS is not configured yet)."
fi
